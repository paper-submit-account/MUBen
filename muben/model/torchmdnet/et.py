...
class EquivariantLayerNorm(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        reg_matrix = torch.diag(torch.tensor([1.0, 2.0, 3.0])).unsqueeze(0).to(inputs.device).type(inputs.dtype)
        covar = self.covariance(inputs) + self.eps * reg_matrix
        covar_sqrtinv = self.symsqrtinv(covar)
        return (covar_sqrtinv @ inputs).to(self.weight.dtype) * self.weight.reshape(1, 1, self.normalized_shape[0])
