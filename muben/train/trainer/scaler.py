...
class StandardScaler:
    ...
    def inverse_transform(self, x: np.ndarray, var: Optional[np.ndarray] = None):
        ...
        transformed_x = np.where(np.isnan(transformed_x), self.replace_nan_token, transformed_x)
        if var is None:
            return transformed_x

        transformed_var = var * self.stds**2
        transformed_var = np.where(np.isnan(transformed_var), self.replace_nan_token, transformed_var)
