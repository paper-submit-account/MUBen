...
class Arguments:
    ...
    model_name: Optional[str] = field(default=None, metadata={"choices": MODEL_NAMES, "help": "A list of model names"})
    ...
    result_seed: Optional[int] = field(
        default=0,
        metadata={"help": "the seed the models are trained with. This argument is used to locate results."},
    )
    ...
    def __post_init__(self):
        ...
        if self.data_seeds is None:
            self.data_seeds: list[int] = [0, 1, 2]
        elif isinstance(self.data_seeds, int):
            self.data_seeds: list[int] = [self.data_seeds]
    ...
    for dataset_name in args.dataset_names:
        ...
        for uncertainty_method in args.uncertainty_methods:
            ...
            for seed in args.data_seeds:
                ...
                test_result_paths = glob.glob(op.join(result_dir, "**", "preds", "*.pt"), recursive=True)
