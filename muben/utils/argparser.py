...
class ArgumentParser(BaseArgumentParser):
    ...
    def parse_dict(self, args: Dict[str, Any]) -> Tuple[DataClass, ...]:
        """
        ...
        for dtype in self.dataclass_types:
            ...
            outputs.append(obj)
        if unused_keys:
            warnings.warn(f"Some keys are not used by the ArgumentParser: {sorted(unused_keys)}")
    ...
    def parse_yaml_file(self, yaml_file: str) -> Tuple[DataClass, ...]:
        ...
        Returns:
            Tuple consisting of:
                ...
                - the dataclass instances in the same order as they were passed to the initializer.
        """
        outputs = self.parse_dict(yaml.safe_load(Path(yaml_file).read_text()))
        return tuple(outputs)
